...
Browser mutations are not retried by transport recovery. Completed execution is logged before the next observation, including when that observation encounters a navigation. StalePage during tick reobserves without ending the run. Two consecutive StalePages on the same chosen control skip that id for one turn and insert a wait so SPA menus can settle. An invalid or empty TypeSafe response is retried once after a fresh observe; a second failure sets BLOCKED with an explicit reason instead of raising. An interrupted native-select evaluation stops because its change event may already have fired. Typing uses a browser select-all command followed by CDP text insertion, so existing input contents are replaced.

The next observation waits for up to two animation frames or 50 ms after an interaction. Editable ARIA comboboxes instead wait for visible options, capped at 200 ms. This avoids paying for a prediction before autocomplete suggestions arrive. Clicking a collapsed combobox, listbox, or `expanded=false` button opener reobserves until options appear or the action list meaningfully changes, capped at 1500 ms, before the next prediction. An explicit WAIT remains 100 ms; network loading is never fast-forwarded in the recording.
